...
Settings::Settings(const uint32_t inQuartzFrequency,
  ...
  if (mDesiredBitRate != inWhishedBitRate) {
    ...
    while ((TQCount >= 5) && (BRP <= 64)) {
      ...
      if (BRP > 0) {
        const uint32_t error =
            clock - inWhishedBitRate * TQCount * BRP;  // error is always >= 0
      ...
      if (BRP < 64) {
        const uint32_t error = inWhishedBitRate * TQCount * (BRP + 1) -
                               clock;  // error is always >= 0
        if (error < smallestError) {
          smallestError = error;
          bestBRP = BRP + 1;
          bestTQCount = TQCount;
    ...
    const uint32_t propSegmentPlusPhaseSegment1 =
        bestTQCount - PS2 - 1 /* Sync Seg */;
    //--- Set PS1 to half of remaining TQCount
    const uint32_t PS1 =
        propSegmentPlusPhaseSegment1 / 2;  // Always 1 <= PS1 <= 8
    mPhaseSegment1 = (uint8_t)PS1;
    //--- Set PS to what is left
    mPropagationSegment = (uint8_t)(propSegmentPlusPhaseSegment1 -
                                    PS1);  // Always 1 <= PropSeg <= 8
    //--- Set SJW to PS2, with a maximum value of 4
    mSJW = (mPhaseSegment2 > 4)
               ? 4
               : (mPhaseSegment2 -
                  1);  // Always 2 <= RJW <= 4, and RJW <= mPhaseSegment2
                       //--- Triple sampling ?
    ...
    const uint64_t ppm =
        (uint64_t)(1000UL * 1000UL);  // UL suffix is required for Arduino Uno
    mBitRateClosedToDesiredRate =
        (diff * ppm) <= (((uint64_t)W) * inTolerancePPM);
  }
};
...
static uint32_t bitrateFrom(const uint32_t inQuartzFrequency,    // In Hertz
  ...
  const uint8_t TQ =
      1 + inPropagationSegment + inPhaseSegment1 + inPhaseSegment2;
  return inQuartzFrequency / inBitRatePrescaler / TQ / 2;
}

//----------------------------------------------------------------------------------------------------------------------

Settings::Settings(const uint32_t inQuartzFrequency,    // In Hertz
                   const uint8_t inBitRatePrescaler,    // 1...64
                   const uint8_t inPropagationSegment,  // 1...8
                   const uint8_t inPhaseSegment1,       // 1...8
                   const uint8_t inPhaseSegment2,       // 2...8
    ...
    :  // 1...4
      mQuartzFrequency(inQuartzFrequency),
      mDesiredBitRate(bitrateFrom(inQuartzFrequency, inBitRatePrescaler,
                                  inPropagationSegment, inPhaseSegment1,
                                  inPhaseSegment2)),
...
uint32_t Settings::actualBitRate(void) const {
  const uint32_t TQCount =
      1 /* Sync Seg */ + mPropagationSegment + mPhaseSegment1 + mPhaseSegment2;
  return mQuartzFrequency / mBitRatePrescaler / TQCount / 2;
}

//----------------------------------------------------------------------------------------------------------------------

bool Settings::exactBitRate(void) const {
  const uint32_t TQCount =
      1 /* Sync Seg */ + mPropagationSegment + mPhaseSegment1 + mPhaseSegment2;
  return mQuartzFrequency ==
         (mDesiredBitRate * mBitRatePrescaler * TQCount * 2);
}

//----------------------------------------------------------------------------------------------------------------------

uint32_t Settings::ppmFromDesiredBitRate(void) const {
  const uint32_t TQCount =
      1 /* Sync Seg */ + mPropagationSegment + mPhaseSegment1 + mPhaseSegment2;
  const uint32_t W = TQCount * mDesiredBitRate * mBitRatePrescaler * 2;
  const uint64_t diff =
      (mQuartzFrequency > W) ? (mQuartzFrequency - W) : (W - mQuartzFrequency);
  const uint64_t ppm =
      (uint64_t)(1000UL * 1000UL);  // UL suffix is required for Arduino Uno
...
uint32_t Settings::samplePointFromBitStart(void) const {
  const uint32_t TQCount =
      1 /* Sync Seg */ + mPropagationSegment + mPhaseSegment1 + mPhaseSegment2;
  const uint32_t samplePoint =
      1 /* Sync Seg */ + mPropagationSegment + mPhaseSegment1 - mTripleSampling;
...
uint16_t Settings::CANBitSettingConsistency(void) const {
